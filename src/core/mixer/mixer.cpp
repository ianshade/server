...
struct mixer::impl
{
    ...
    std::queue<std::map<pixel_format, std::shared_future<const_frame>>> buffer_;
                                                                    ...
                                                                    std::vector<core::pixel_format> pixel_formats)
    {
        ...
        auto image = (*image_mixer_)(format_desc, pixel_formats);
        auto audio = audio_mixer_(format_desc, nb_samples);

        state_["audio"] = audio_mixer_.state();

        std::map<pixel_format, std::shared_future<const_frame>> multiframe;
        for (auto& pixel_format : pixel_formats) {
            multiframe.insert(std::pair<core::pixel_format, std::shared_future<const_frame>>(
                pixel_format,
                std::async(std::launch::deferred,
                           [image = std::move(image[pixel_format]),
                            ...
                            tag = this]() mutable {
                               ...
                               desc.planes.push_back(
                                   pixel_format_desc::plane(format_desc.width, format_desc.height, 4));
        ...
        buffer_.push(std::move(multiframe));
        std::map<pixel_format, const_frame> result;
        if (buffer_.size() < 2) {
            for (auto& pixel_format : pixel_formats) {
                result.insert(std::pair<core::pixel_format, const_frame>(
                    pixel_format,
                    const_frame{}));
            }
        } else {
            auto multiframe2 = buffer_.front();

            for (auto& frame : multiframe2) {
                result[frame.first] = std::move(frame.second.get());
            }
            buffer_.pop();
        }
        return spl::make_shared<std::map<pixel_format, const_frame>>(result);
    }
...
spl::shared_ptr<std::map<pixel_format, const_frame>> mixer::operator()(std::vector<draw_frame>         frames,
                                                                       const video_format_desc&        format_desc,
                                                                       int                             nb_samples,
                                                                       std::vector<core::pixel_format> pixel_formats)
{
    return (*impl_)(std::move(frames), format_desc, nb_samples, pixel_formats);
}
